...
def sum_groups(dataframe, combined_groups, summing_groups):
    for idx in range(len(combined_groups)):
        dataframe[combined_groups[idx]] = \
            dataframe.loc[:, summing_groups[idx]].sum(axis=1)
    return dataframe
...
def create_model(model_type, units, hidden_layer_units):
    ...
    if model_type == "GRU":
        if CONDITIONAL_RNN:
            model_to_create.add(ConditionalRecurrent(layers.GRU(units)))
        else:
            model_to_create.add(layers.GRU(units))
    elif model_type == "LSTM":
        if CONDITIONAL_RNN:
            model_to_create.add(ConditionalRecurrent(layers.LSTM(units)))
        else:
    ...
    for UNITS in range(3, 5):
        for HIDDEN_LAYER_UNITS in range(3, 5):
            ...
            results.append(math.sqrt(
                mean_squared_error(scaler.inverse_transform(train_response),
...
for i in range(3):
    for j in range(4):
        ...
        axs[i, j].plot(x, scaler.inverse_transform(
            dataset.iloc[61 * (2 * i + int(PLOT_WOMEN)):61 * (2 * i + 1 + int(PLOT_WOMEN)), 4:])[:61, agegroup],
...
for j in range(3):
    for i in range(61):
        axs[j, 0].plot(x, np.log(scaler.inverse_transform(dataset.iloc[:, 4:]).transpose(1, 0))[:, 61 * j * 2 + i],
                       color=colors[i], linewidth=0.5)
        axs[j, 1].plot(x,
                       np.log(scaler.inverse_transform(dataset.iloc[:, 4:]).transpose(1, 0))[:, 61 * (j * 2 + 1) + i],
                       color=colors[i], linewidth=0.5)
...
labels = ["Lithuanian males", "Lithuanian females", "Latvian males", "Latvian females", "Estonian males", "Estonian "
                                                                                                          "females"]
